      ...
      <CardContent className="relative px-6 pb-6">
        <div className="flex flex-col md:flex-row gap-6 -mt-12 items-start">
          ...
          <div className="relative">
            ...
            {user.isVerified && (
              <div className="absolute bottom-2 right-2 bg-white p-1.5 rounded-full shadow-sm" title="Verified ID">
          ...
          <div className="flex-1 pt-2 md:pt-14 space-y-4">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                ...
                <p className="text-slate-500 text-sm flex items-center gap-2 mt-1">
                  <span>{user.age} Years</span> • <span>{user.gender}</span> • <span>Member since {new Date(user.createdAt).getFullYear()}</span>
                </p>
              </div>
              ...
              <div className="flex gap-4">
                <div className="text-center p-3 bg-slate-50 rounded-lg border border-slate-100 min-w-[80px]">
                  <p className="text-xl font-bold text-slate-900">{user._count.tripsCreated}</p>
                  <p className="text-xs text-slate-500 font-medium uppercase">Trips</p>
                </div>
                <div className="text-center p-3 bg-slate-50 rounded-lg border border-slate-100 min-w-[80px]">
                  <p className="text-xl font-bold text-slate-900">{user._count.reviewsReceived}</p>
                  <p className="text-xs text-slate-500 font-medium uppercase">Reviews</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {user.interests.map((interest) => (
                <Badge key={interest} variant="secondary" className="bg-slate-100 text-slate-600 hover:bg-slate-200">
                  {interest}
                </Badge>
              ))}
            </div>
